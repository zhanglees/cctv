...
public class ExcelUtils {
    ...
    public static void writeOperateTagToExcel(String fileName, List<OperateBean> maps) {
        fileName = fileName.trim() + FILE_SUFFIX;
        initExcel(fileName, mOperateData);
        writeOperateTagListToExcel(maps, fileName);
    }

    public static void writeTagToExcel(String fileName, List<InventoryBean> maps) {
        ...
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ...
                    InventoryBean bean = ((InventoryBean) (objList.get(j)));
                    list.add(j + 1 + "");
                    list.add(bean.getEpc());
                    list.add(bean.getPc());
                    list.add(bean.getTimes() + "");
                    list.add((Integer.parseInt(bean.getRssi()) - 129) + "dBm");
                    list.add(bean.getFreq());
                    list.add(new SimpleDateFormat("yyyy/MM/dd").format(bean.getDate()) + " " + new SimpleDateFormat("HH:mm:ss").format(bean.getDate()));
    ...
    public static <T> void writeOperateTagListToExcel(List<T> objList, String fileName) {
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ArrayList<String> list = new ArrayList<String>();
                    OperateBean bean = (OperateBean) (objList.get(j));
                    list.add(j + 1 + "");
                    list.add(bean.getPc());
                    list.add(bean.getCrc());
                    list.add(bean.getEpc());
                    list.add(bean.getData());
                    list.add(bean.getDataLen() + "");
                    list.add(bean.getAntId() + "");
                    list.add(bean.getTimes() + "");
                    for (int i = 0; i < list.size(); i++) {
                        sheet.addCell(new Label(i, j + 1, list.get(i),
                                arial12format));
