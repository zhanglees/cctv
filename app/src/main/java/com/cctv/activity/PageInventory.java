...
public class PageInventory extends LinearLayout {
    ...
    private TagCarList mTagRealList;
    ...
    private List<OrderItemsBean> orderList = new ArrayList<>();
    ...
    public PageInventory(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        mApp = (UHFApplication) ((Activity) context).getApplication();
        LayoutInflater.from(context)
                .inflate(layout.page_inventory, this);
        try {
        ...
        mLogList = (LogList) findViewById(id.car_log_list);
        mStartStop = (TextView) findViewById(id.car_startstop);

        //mSessionIdTextView = findViewById(id.car_session_id_text);
        //mInventoriedFlagTextView = findViewById(id.car_inventoried_flag_text);
        //mDropDownRow1 = findViewById(id.car_table_row_session_id);
        //mDropDownRow2 = findViewById(id.car_table_row_inventoried_flag);
        mTagsCountText = findViewById(id.car_tags_count_text);
        mTagsTotalText = findViewById(id.car_tags_total_text);
        mTagsSpeedText = findViewById(id.car_tags_speed_text);
        mTagsTimeText = findViewById(id.car_tags_time_text);
        mTagsOpTimeText = findViewById(id.car_tags_op_time_text);
        mTagRealList = findViewById(id.car_tag_real_list);
        mRealRoundEditText = findViewById(id.car_real_round_text);

        mStartStop.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (v.getId() == id.car_startstop) {
       ...
       // setInventoriedFlagText(mPos2);
        ...
        //mSelectCommand = findViewById(id.car_comand_select_radio);
        ...
        ((CarLoadActivity) mContext).mInventoryList = mTagRealList.getData();
    }
   ...
   // }
            ...
            mLoopHandler.post(new Runnable() {
                ...
                public void run() {
                 //   Log.i("obj",""+orderList.size());
                  ...
                  String epccode = bean.getEpc().replaceAll(" ", "").toLowerCase();
                   // Log.i("epc",epccode+ orderList.size());
                    orderList =  ((CarLoadActivity) mContext).ordersInfo;
                    if(orderList!=null && orderList.size()>0)
                    {
                        for(OrderItemsBean ord :orderList)
                        {
                            Log.i("epc",ord.getEpcnum());
                            if(epccode.equals(ord.getEpcnum().toLowerCase()))
                            {
    ...
    public  void setOrderList(List<OrderItemsBean> datas)
    {
        if(datas!=null)
