...
import com.cctv.device.BaseActivity;
import com.cctv.device.MainActivityList;
import com.cctv.device.MonitorActivity;
import com.cctv.device.PageInventoryReal;
import com.cctv.device.PageTagAccess;
...
public class CarLoadActivity extends BaseActivity {
    ...
    public List<OrderItemsBean> ordersInfo = new ArrayList<>();
    String tasknum = "";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ordersInfo =  (List<OrderItemsBean>) getIntent().getSerializableExtra("orderitemsbean");
        ...
        mRefreshButton = (TextView) findViewById(R.id.refreshData);
        mRefreshButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (cur_item == PopupMenu.MENUITEM.ITEM1) {
                    if (0 == currIndex) {
                        ((PageInventory) findViewById(R.id.view_PageInventory)).refresh();
                        ((PageInventory) findViewById(R.id.view_PageInventory)).setOrderList(ordersInfo);
                    } else {
        ...
        iv_menu = (ImageView) findViewById(R.id.iv_menuCar);
        iv_menu.setOnClickListener(new OnClickListener() {

            @Override
            public void onClick(View v) {
                // TODO Auto-generated method stub
                popupMenu.showLocation(R.id.iv_menuCar);
    ...
    private void InitTextView() {
        title[0] = (TextView) findViewById(R.id.tab_showtask);
        title[1] = (TextView) findViewById(R.id.tab_taskum);

        title[0].setText(tasknum);
        title[0].setOnClickListener(new CarLoadActivity.MyOnClickListener(0));
        title[1].setOnClickListener(new CarLoadActivity.MyOnClickListener(1));
    }

    private void InitViewPager(PopupMenu.MENUITEM item) {
        cur_item = item;
        mPager = (ViewPager) findViewById(R.id.vPagerCar);
        listViews = new ArrayList<>();
        LayoutInflater mInflater = getLayoutInflater();
        if (item == PopupMenu.MENUITEM.ITEM1) {
            listViews.add(mInflater.inflate(R.layout.carlay, null));
            //listViews.add(mInflater.inflate(R.layout.lay2, null));
        }
        mPager.setAdapter(new CarLoadActivity.MyPagerAdapter(listViews));
        mPager.setCurrentItem(0);
        mPager.setOnPageChangeListener(new CarLoadActivity.MyOnPageChangeListener());
        currIndex = 0;
            ...
            case KeyEvent.KEYCODE_F4: {
                if (cur_item == PopupMenu.MENUITEM.ITEM1) {
                    if (0 == currIndex) {
                        if (!mKeyF4Pressing) {
                            mKeyF4Pressing = true;
                            ((PageInventory) findViewById(R.id.view_PageInventory)).startStop(true);
                        }
                    }
                } else if (cur_item == PopupMenu.MENUITEM.ITEM2) {
                    if (0 == currIndex) {
        ...
        if (keyCode == KeyEvent.KEYCODE_MENU) {
            ...
            if (cur_item == MENUITEM.ITEM1) {
                if (0 == currIndex) {
                    ...
                    ((PageInventory) findViewById(R.id.view_PageInventory)).startStop(false);
                }
                ...
                setPositiveButton(getString(R.string.ok),
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                PreferenceUtil.commitInt(CarLoadActivity.REGION_TYPE, m_curReaderSetting.btRegion);
                                PreferenceUtil.commitInt(CarLoadActivity.FREQUENCY_INTERVAL, m_curReaderSetting.btUserDefineFrequencyInterval);
                                PreferenceUtil.commitInt(CarLoadActivity.START_FREQUENCY, m_curReaderSetting.nUserDefineStartFrequency);
                                //close the module
    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (level < 10) {
                if (mWornDialog == null) {
                    mWornDialog = new AlertDialog.Builder(CarLoadActivity.this)
                            .setCancelable(false)
                            .setTitle("警告")
                            .setMessage("当前电量低于10%")
